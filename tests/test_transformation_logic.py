    ...
    mock_reviews = [
        {
            ...
            "products": [{"product": {"sku": "TEST-SKU-001", "title": "Test Product 1"}}],
        }
    ]

    mock_product_response = {"products": [{"sku": "TEST-SKU-001", "rating": {"rating": 4.5, "count": 10}}]}
...
def test_sku_deduplication() -> None:
    ...
    mock_reviews = [
        {
            "url": "https://feefo.com/review/1",
            "products": [{"product": {"sku": "DUPLICATE-SKU", "title": "Product A"}}],
        },
        {
        ...
        {"url": "https://feefo.com/review/3", "products": [{"product": {"sku": "UNIQUE-SKU", "title": "Product B"}}]},
    ]

    mock_product_response = {"products": [{"sku": "TEST", "rating": {"rating": 4.5}}]}
    ...
    with patch("requests.get") as mock_get:
        ...
        called_skus = [call.kwargs["params"]["product_sku"] for call in mock_get.call_args_list]
...
def test_multiple_products_in_single_review() -> None:
    ...
    mock_product_response = {"products": [{"sku": "TEST", "rating": {"rating": 5.0}}]}
...
def test_period_days_parameter_propagation() -> None:
    ...
    """
    mock_reviews = [{"url": "https://feefo.com/review/1", "products": [{"product": {"sku": "TEST-SKU"}}]}]

    mock_product_response = {"products": [{"sku": "TEST-SKU", "rating": {"rating": 4.0}}]}
    ...
    with patch("requests.get") as mock_get:
        ...
        list(
            fetch_products_from_reviews(
                merchant_id="test-merchant", reviews_resource=iter(mock_reviews), period_days=30
            )
        )
...
def test_missing_sku_is_skipped() -> None:
    ...
    mock_reviews = [
        {"url": "https://feefo.com/review/1", "products": [{"product": {"sku": None, "title": "Product without SKU"}}]},
        {
            "url": "https://feefo.com/review/2",
            "products": [
        ...
        },
        {
            ...
            "products": [{"product": {"sku": "VALID-SKU", "title": "Valid Product"}}],
        },
    ]

    mock_product_response = {"products": [{"sku": "VALID-SKU", "rating": {"rating": 5.0}}]}
...
def test_empty_products_array_is_handled() -> None:
    ...
    mock_reviews = [
        ...
        {"url": "https://feefo.com/review/2", "products": [{"product": {"sku": "VALID-SKU"}}]},
    ]

    mock_product_response = {"products": [{"sku": "VALID-SKU", "rating": {"rating": 4.5}}]}
...
def test_period_days_formatting(period_days: int, expected_param: str) -> None:
    ...
    with patch("requests.get") as mock_get:
        ...
        list(
            fetch_products_from_reviews(
                merchant_id="test-merchant", reviews_resource=iter(mock_reviews), period_days=period_days
            )
        )
